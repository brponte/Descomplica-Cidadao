$( function ()
{
	var ID_STATUS_DEMANDA_INVALIDO = "-1";

	var $form = $("#form-preencher-lista");

	//filtros
	var $cmbFiltroStatusDemanda = $form.find("#select-status-demanda");
	...
	var inicializar = function ()
	{
		...
		$( "#tabela-demanda" ).find( ".a-link-assumir" )
			.off( "click" )
			.on( "click", function ( ev )
			{
				...
				Common.messageBoxSimNao($, $.TITULO_DLG, Common.formatarString($.MSG_DEMANDA_CONFIRMA_ASSUMIR, idDemanda ), function () { fnConfirmaAssumir( $link ); }, null );

				return ( false );
			} );

		$("#tabela-demanda").find(".a-link-finalizar")
			.off("click")
			.on("click", function (ev) {
				if (ev)
					ev.preventDefault();
				...
				Common.messageBoxSimNao($, $.TITULO_DLG, Common.formatarString($.MSG_DEMANDA_CONFIRMA_FINALIZAR, idDemanda), function () { fnConfirmaFinalizar($link); }, null);
	...
	var fnConfirmaAssumir = function ( $link )
	{
	...
	var fnConfirmaFinalizar = function ($link) {
		var strURL = $link.attr("href");
	...
	var onFormFiltrar = function (ev) {
		...
		if (ehFormValido()) {
			...
			var idStatusDemanda = $cmbFiltroStatusDemanda.val();

			var dataToSend = "filtroStatusDemanda=" + idStatusDemanda;
	...
	var ehFormValido = function () {
		...
		//Status da Demanda
		var idStatusDemanda = $cmbFiltroStatusDemanda.val();
		if (idStatusDemanda == ID_STATUS_DEMANDA_INVALIDO) {
			exibirErro($.ERR_CAMPO_STATUS_DEMANDA_OBRIGATORIO);
			return (false);
		}
